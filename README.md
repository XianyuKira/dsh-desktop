...
| 状态栏 | 服务状态、端口、**插件市场状态**、工作目录、缩放比例 |

### 状态栏的插件市场

状态栏常驻显示插件市场（`dshmarket`）的状态，不用每次进「设置 → 插件市场」去看：
...
**鼠标悬停**能看到明细，例如哪两个插件、从哪个版本升到哪个版本；**点一下**打开
市场（需要界面已加载完成）。

实现上它读的是市场自带的本地只读 API（`GET /dsh-market/api/v1/capabilities`、
`/dsh-market/status`、`/dsh-market/api/v1/updates?name=…`），无需会话令牌。更新检查
（`updates`）比较贵——每个插件都可能去问一次仓库——所以它有独立节奏：
每 8 秒读一次便宜的状态，完整扫更新每 10 分钟一次；市场路由还没挂上时（dsh 启动期间）
会自动重试，不会误报成「未启用」。
